...
% To generate a new test signal or not. Already generated signal supports up to 6 sensors
genSig = false;
...
for i = 1:num_samples
    counter = counter + 1
    distanceFiltered = FIRfilter(distanceNoisy, i);
end
...
for i = TimeStep - n: TimeStep
    ...
    index = i + n - TimeStep + 1;
    estimate = estimate + Sensordata(1, i) * filter(index);
end
