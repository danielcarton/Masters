...
%     if i == 1
%         realTemp(i) = tempArray(i);
%     end
%     if i > 1
%         realTemp(i) = realTemp(i-1) + tempArray(i);
%     end
% end  

% for i = i:size(realTemp)
%     realTemp[i] = realTemp[i-1] + tempArray;
% end
...
% Kalman
Xsaved = zeros(samples, 1);

for i = 1:samples
    x = kalman(noisyTemp(i));
...
nexttile;
plot(reducedTimeSpace, Xsaved)
%plot(reducedTimeSpace, algOutValueNoise);
title("Noisy temp kalman filtered");
